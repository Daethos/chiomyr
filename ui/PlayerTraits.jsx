...
export const getAsceanTraits = async (ascean) => {
    let traits = {
        primary: {},
        secondary: {},
        tertiary: {},
...
export const checkPlayerTrait = (trait, gameState) => {
...
export const checkTraits = (trait, traits) => {
...
export const LuckoutModal = ({ traits, callback, name, influence, show, setShow }) => {
    const orientation = useDeviceOrientation();
    return (
        <>
        <Modal visible={show} onRequestClose={() => setShow(false)} style={{ zIndex: 1, top: '-25%' }}>
        <TouchableOpacity onPress={() => setShow(true)} style={[styles.border, styles.popover, { maxHeight: orientation === 'landscape' ? '75%' : '50%', maxWidth: orientation === 'landscape' ? '50%' : '70%' }]}>
        <View style={{ height: '100%' }}>
        <Text style={{ textAlign: 'center', fontSize: "1.25em", color: "gold" }}>Hush and Tendril</Text>
        <Text style={{ textAlign: 'center' }}>
            ...
            <View style={{ fontSize: "18px", color: "gold" }}>
            {traits.map((trait, index) => {
                return (
                    <View key={index}>
                        <TouchableOpacity style={styles.dialogButtonsInner} onPress={() => callback(trait.name)}>
                            <Text style={{ color: traitStyle(trait.name), fontSize: '1em' }}>
                            [{trait.name}] - {trait.luckout.modal.replace('{enemy.name}', name).replace('{ascean.weapon_one.influences[0]}', influence)}
                            </Text>
                        </TouchableOpacity>
                    </View>
                )
        ...
        <TouchableOpacity style={styles.dialogButtonsInner} onPress={() => setShow(true)}>
            <Text style={{ color: "pink" }}>
            [ {'>>>'} Combat Alternative(s) {'<<<'} ]
            </Text>
        </TouchableOpacity>
        {traits.map((trait, index) => {
            return (
                <div key={index}>
                    <TouchableOpacity style={styles.dialogButtonsInner} onPress={() => callback(trait.name)}>
                        <Text style={{ color: traitStyle(trait.name) }}>
                        [{trait.name}] - {trait.luckout.action.replace('{enemy.name}', name).replace('{ascean.weapon_one.influences[0]}', influence)}
                        </Text>
                    </TouchableOpacity>
                </div>
            )
...
export const PersuasionModal = ({ traits, callback, name, influence, show, setShow }) => {
    const orientation = useDeviceOrientation();
    return (
        <>
        <Modal visible={show} onRequestClose={() => setShow(false)} style={{ zIndex: 1, top: '-25%' }}>
        ...
        <View style={{ height: '100%' }}>
        <Text style={{ textAlign: 'center', fontSize: "1em", color: "gold" }}>Correspondence</Text>
        <Text style={{ textAlign: 'center' }}>
            ...
            {traits.map((trait, index) => {
                return (
                    <View key={index}>
                        <TouchableOpacity style={styles.dialogButtonsInner} onPress={() => callback(trait.name)}>
                            <Text style={{ color: traitStyle(trait.name), fontSize: '1.25em' }}>
                            [{trait.name}]: {trait.persuasion.modal.replace('{enemy.name}', name).replace('{ascean.weapon_one.influences[0]}', influence)}
                            </Text>
                        </TouchableOpacity>
                    </View>
                )
        ...
        {traits.map((trait, index) => {
            return (
                ...
                <TouchableOpacity style={styles.dialogButtonsInner} onPress={() => callback(trait.name)}>
                    <Text style={{ color: traitStyle(trait.name) }}>
                        [{trait.name}]: {trait.persuasion.action.replace('{enemy.name}', name).replace('{ascean.weapon_one.influences[0]}', influence)}
                    </Text>
                </TouchableOpacity>
            </View>
            )
